...
@pytest.fixture
def maquina_estados(
    mock_repositorio_flujo: MockRepositorioFlujo,
    mock_repositorio_clientes: MockRepositorioClientes,
    mock_extractor_necesidad: MockExtractorNecesidad,
    mock_buscador_proveedores: MockBuscadorProveedores,
    mock_servicio_consentimiento: MockServicioConsentimiento,
    mock_logger: MagicMock,
) -> "MaquinaEstados":
    """Provides a configured state machine instance."""
    from state_machine import MaquinaEstados

    return MaquinaEstados(
        repositorio_flujo=mock_repositorio_flujo,
        repositorio_clientes=mock_repositorio_clientes,
        extractor_necesidad=mock_extractor_necesidad,
        buscador_proveedores=mock_buscador_proveedores,
        servicio_consentimiento=mock_servicio_consentimiento,
        logger=mock_logger,
    )
...
@pytest.fixture
def contexto_base(
    flujo_factory: FlujoFactory,
    mock_repositorio_flujo: MockRepositorioFlujo,
    mock_logger: MagicMock,
) -> "ContextoConversacionState":
    """Provides a basic conversation context for testing."""
    from state_machine.contexto import ContextoConversacionState

    flujo = flujo_factory.en_servicio()
    return ContextoConversacionState(
        flujo=flujo,
        telefono=flujo.telefono,
        texto_mensaje="test message",
        repositorio_flujo=mock_repositorio_flujo,
        logger=mock_logger,
    )
