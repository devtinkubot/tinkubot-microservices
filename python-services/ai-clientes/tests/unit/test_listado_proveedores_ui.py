# flake8: noqa
"""Tests para la UI del listado de proveedores."""
...
def test_mensaje_intro_listado_proveedores_usa_copy_nuevo():
    assert (
        mensaje_intro_listado_proveedores("Cuenca")
        == "*Encontré estas opciones en Cuenca:*"
    )


def test_construir_ui_lista_proveedores_limita_a_cinco():
    proveedores = [
        {"id": f"prov-{idx}", "name": f"Proveedor {idx}"} for idx in range(1, 8)
    ]
    ...
    assert ui["footer_text"] == "Tienes 5 minutos para elegir un experto."
...
async def test_presenting_results_acepta_interactive_list_reply():
    ...
    assert resultado["messages"][0]["ui"] == {
        "type": "list",
...
async def test_presenting_results_con_texto_libre_reenvia_lista_interactiva():
    ...
    assert (
        resultado["response"] == "Selecciona un experto de la lista para ver su perfil."
    )
