"""
Saga Pattern Implementation for Provider Registration.
...
class ProviderRegistrationSaga:
    """
    Orchestrates provider registration with automatic rollback.

    This saga executes a sequence of commands to register a provider, ensuring
    that if any step fails, all previously executed steps are rolled back in
    reverse order. This provides transactional semantics across multiple
    operations (database, storage, external services, etc.).
    ...
    Example:
        ...
        ...     .add_command(UploadDniFrontCommand(image_service, provider_id, image))
        ...     .add_command(UploadDniBackCommand(image_service, provider_id, image)))
        >>>
    ...
    def __init__(self):
        """
        Initialize the ProviderRegistrationSaga.
        ...
        Example:
            >>> saga = ProviderRegistrationSaga()
            >>> saga.add_command(RegisterProviderCommand(repo, data))
        """
        self.commands: List[Command] = []
        self.executed_commands: List[Command] = []

        logger.debug("ProviderRegistrationSaga initialized")
    ...
    def add_command(self, command: Command) -> 'ProviderRegistrationSaga':
        ...
        Example:
            ...
            ...     .add_command(UploadDniFrontCommand(img_svc, pid, img))
            ...     .add_command(UploadDniBackCommand(img_svc, pid, img)))
        """
    ...
    async def execute(self) -> Dict[str, Any]:
        ...
        try:
            ...
            raise SagaExecutionError(
                f"Registration failed at step {failed_index + 1}: {str(e)}",
    ...
    async def _rollback(self) -> None:
        ...
        registered a provider, uploaded DNI front, and then DNI back failed,
        we undo DNI front first, then provider registration.
    ...
    def reset(self) -> None:
        ...
        the saga instance to be reused for a new registration.
        ...
        Example:
            >>> saga.reset()
            >>> saga.add_command(RegisterProviderCommand(repo, new_data))
