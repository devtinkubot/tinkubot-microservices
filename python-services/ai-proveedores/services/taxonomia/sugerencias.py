...
async def registrar_sugerencia_taxonomia(
    *,
    ...
    taxonomia = await obtener_taxonomia_publicada(supabase, redis_client=redis_client)
...
async def _buscar_sugerencia_existente(supabase: Any, normalized_text: str, source_channel: str) -> Optional[Dict[str, Any]]:
    respuesta = await run_supabase(
        ...
        .limit(1)
        .execute(),
        label="taxonomy_suggestions.by_normalized",
    )
...
async def _insertar_sugerencia(supabase: Any, payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    respuesta = await run_supabase(
        ...
        label="taxonomy_suggestions.insert",
    )
    data = getattr(respuesta, "data", None) or []
    return data[0] if data else None


async def _actualizar_sugerencia(supabase: Any, suggestion_id: str, payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    respuesta = await run_supabase(
        lambda: supabase.table("service_taxonomy_suggestions")
        .update(payload)
        .eq("id", suggestion_id)
        .execute(),
        label="taxonomy_suggestions.update",
    )
    data = getattr(respuesta, "data", None) or []
    return data[0] if data else None
