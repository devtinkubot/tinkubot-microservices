    ...
    for servicio in servicios_limpios:
        ...
        resolved_domain_code = (
            ...
            if normalized_domain and (
                not codigos_catalogo or normalized_domain in codigos_catalogo
            )
            else None
        )
        category_name = (
        ...
        status = str(fila.get("domain_resolution_status") or "").strip().lower()
        if not status:
            status = "matched" if resolved_domain_code else "clarification_required"
        resultados.append(
            {
                ...
                "is_valid_service": bool(fila.get("is_valid_service")),
                "needs_clarification": bool(fila.get("needs_clarification")),
                "reason": fila.get("reason"),
                "clarification_question": fila.get("clarification_question"),
            }
        )
    return resultados
