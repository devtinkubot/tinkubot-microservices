...
async function obtenerPerfilProfesionalIncompleto(req, res) {
  try {
    const providers =
      await proveedoresBff.obtenerProveedoresPerfilProfesionalIncompleto();
    res.json({ providers });
  } catch (error) {
    const status = error?.status ?? 500;
    const payload = error?.data ?? {
      error:
        "No se pudo obtener la lista de perfiles profesionales incompletos.",
    };
    res.status(status).json(payload);
  }
}
...
async function aprobarProveedor(req, res) {
  try {
    ...
    const resultado = await proveedoresBff.aprobarProveedor(
      providerId,
      req.body ?? {},
      requestId,
    );
...
async function rechazarProveedor(req, res) {
  try {
    ...
    const requestId =
      req.headers["x-request-id"] || req.headers["x-correlation-id"] || null;
    const resultado = await proveedoresBff.rechazarProveedor(
      providerId,
      req.body ?? {},
      requestId,
    );
...
async function obtenerResumenEstados(req, res) {
  try {
    const overview = await proveedoresBff.obtenerResumenEstadosProveedores();
    res.json(overview);
  } catch (error) {
    const status = error?.status ?? 500;
    const payload = error?.data ?? {
      error: "No se pudo obtener el resumen de estados de proveedores.",
    };
    res.status(status).json(payload);
  }
}
...
async function obtenerMonetizacionProveedor(req, res) {
  try {
    const { providerId } = req.params;
    const resultado =
      await proveedoresBff.obtenerMonetizacionProveedor(providerId);
    res.json(resultado);
  } catch (error) {
    const status = error?.status ?? 500;
    const payload = error?.data ?? {
